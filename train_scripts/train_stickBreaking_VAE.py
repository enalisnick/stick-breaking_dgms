...
from models.StickBreaking_VAE import StickBreaking_VAE
...
### Train & Evaluate ###
def train_and_eval_stickBreaking_vae(
    dataset,
    hidden_layer_sizes,
    hidden_layer_types,
    latent_size,
    ...
    # construct the Stick-Breaking Variational Autoencoder
    model = StickBreaking_VAE(rng=rng, input=x, batch_size=batch_size, layer_sizes=layer_sizes, layer_types=hidden_layer_types, 
    ...
    results_file_name = pjoin(experiment_dir, "stickBreaking_vae_results_"+output_file_base_name+".txt")
    file_exists_counter = 0
    while os.path.isfile(results_file_name):
        file_exists_counter += 1
        results_file_name = pjoin(experiment_dir, "stickBreaking_vae_results_"+output_file_base_name+"_"+str(file_exists_counter)+".txt")
    ...
    for epoch_counter in range(n_epochs):
        ...
        if valid_error < best_valid_error:
            ...
            cPickle.dump(best_params, open(pjoin(experiment_dir, 'stickBreaking_vae_params_'+output_file_base_name+'.pkl'), 'wb'), protocol=cPickle.HIGHEST_PROTOCOL)
